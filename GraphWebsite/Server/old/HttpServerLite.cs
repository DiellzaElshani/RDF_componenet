...
//	{
//		public HttpServerLite.Webserver _Server;

//		public void StartServer(int port, string graph_db_url, string userAcc, string userPwd)
//		{
//			string AUTH = "Basic " + System.Convert.ToBase64String(Encoding.ASCII.GetBytes($"{userAcc}:{userPwd}"));
...
//			//Webserver server = new Webserver("localhost", port, false, null, null, MyGraphWebsite);
//			//server.Start();

//			Webserver server = new Webserver("localhost", port, false, null, null, HandleRequest);
//			server.Start();
//		}
...
//			//if (ctx.Request.Url.Full == "/upload" && ctx.Request.Method == HttpMethod.GET)
//			if (ctx.Route.Equals("/upload") && ctx.Request.Method == HttpMethod.GET)
//			{
//				await HandleFileUpload(ctx);
//			}
//			//else if (ctx.Request.Url.Full == "/sparql" && ctx.Request.Method == HttpMethod.GET)
//			else if (ctx.Route.Equals("/sparql") && ctx.Request.Method == HttpMethod.GET)
//			{
//				await HandleSparqlQuery(ctx);
//			}
//			else
//			{
//				ctx.Response.StatusCode = 404;
//				await ctx.Response.SendAsync("Endpoint not found");
//			}
//		}

//		// Handle file uploads
//		private static async Task HandleFileUpload(HttpContext ctx)
//		{
//			if (ctx.Request.ContentType.StartsWith("multipart/form-data", StringComparison.OrdinalIgnoreCase))
//			{
//				try
//				{
//					// Extract the uploaded file from the request
//					var formData = await ctx.Request.ReadFormDataAsync();
//					var file = formData.Files["rdfFile"];
//					if (file == null)
//					{
//						ctx.Response.StatusCode = 400;
//						await ctx.Response.SendAsync("No file uploaded.");
//						return;
//					}
...
//						// Send the file content to GraphDB
//						using (HttpClient client = new HttpClient())
//						{
//							var content = new StringContent(fileContent, Encoding.UTF8, "text/turtle");
//							client.DefaultRequestHeaders.Add("Authorization", AUTH);

//							var response = await client.PostAsync($"{GRAPHDB_URL}/statements", content);

//							if (response.IsSuccessStatusCode)
//							{
//								await ctx.Response.SendAsync("File uploaded successfully to GraphDB.");
//							}
//							else
//							{
//								string responseBody = await response.Content.ReadAsStringAsync();
//								ctx.Response.StatusCode = (int)response.StatusCode;
//								await ctx.Response.SendAsync($"Failed to upload file: {responseBody}");
//							}
//						}
//					}
//				}
...
//					await ctx.Response.SendAsync($"Server error: {ex.Message}");
//				}
//			}
//		}

//		// Handle SPARQL queries
//		private static async Task HandleSparqlQuery(HttpContext ctx)
//		{
//			try
//			{
//				string sparqlQuery = ctx.Request.QueryString["query"];
//				string includeInferredData = ctx.Request.QueryString["reasoning"] ?? "false";

//				if (string.IsNullOrEmpty(sparqlQuery))
//				{
//					ctx.Response.StatusCode = 400;
//					await ctx.Response.SendAsync("No SPARQL query provided.");
//					return;
//				}

//				// Create full query URL
//				string fullUrl = $"{GRAPHDB_URL}?query={Uri.EscapeDataString(sparqlQuery)}&infer={includeInferredData.ToLower()}";
...
//						ctx.Response.ContentType = "application/rdf+xml; charset=utf-8";
...
//						ctx.Response.StatusCode = (int)response.StatusCode;
//						await ctx.Response.SendAsync($"SPARQL query failed: {errorText}");
//					}
//				}
//			}
//			catch (Exception ex)
//			{
//				ctx.Response.StatusCode = 500;
//				await ctx.Response.SendAsync($"Error during SPARQL query: {ex.Message}");
//			}
//		}

//		#region
//		//[DynamicRoute(HttpMethod.GET, "^/dynamic/\\d+$")]
//		//public static async Task MyGraphWebsite(HttpContext ctx)
//		//{
//		//          string resp = @"
//		//              <!DOCTYPE html>
//		//              <html lang=""en"">
...
//		/// <summary>
//		/// Examples of different types of Http Get Methods.
//		/// </summary>
